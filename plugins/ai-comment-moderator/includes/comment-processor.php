...
class AI_Comment_Moderator_Comment_Processor {
    ...
    public function process_single_comment($comment_id, $prompt_id) {
        ...
        // Get the comment (could be local or remote)
        ...
        // Apply the action based on comment type
        if (isset($comment->is_remote) && $comment->is_remote) {
            // For remote comments, update local database and sync back
            $this->apply_remote_comment_action($comment->site_id, $comment->remote_comment_id, $decision_data['action']);
    ...
    /**
     * Get comment data from either local or remote source
     * Returns a standardized comment object
     */
    private function get_comment_data($comment_id) {
        global $wpdb;
        
        // First, check if this is a remote comment
        ...
        if ($remote_comment) {
            // Convert remote comment to standard format
            return (object) array(
                ...
                'comment_date_gmt' => $remote_comment->comment_date,
                'comment_content' => $remote_comment->comment_content,
                'comment_karma' => 0,
                'comment_approved' => $remote_comment->comment_status,
                'comment_agent' => '',
                'comment_type' => 'remote',
                'comment_parent' => 0,
                'user_id' => 0,
                ...
                'is_remote' => true,
                'site_id' => $remote_comment->site_id,
                'remote_comment_id' => $remote_comment->remote_comment_id,
                'post_title' => $remote_comment->post_title
            );
        }
        
        // If not remote, get from local WordPress comments
        $comment = get_comment($comment_id);
        if ($comment) {
            // Add flag to indicate this is a local comment
