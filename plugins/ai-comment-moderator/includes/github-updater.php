<?php
/**
 * GitHub Auto-Updater for AI Comment Moderator
...
class AI_Comment_Moderator_GitHub_Updater {
    ...
    public function __construct($plugin_file, $github_repo) {
        ...
        $this->github_token = get_option('ai_comment_moderator_github_token', '');
    ...
    private function get_github_release_info() {
        ...
        $cache_key = 'ai_comment_moderator_github_release';
        ...
        if (is_wp_error($response)) {
            error_log('AI Comment Moderator GitHub Updater: ' . $response->get_error_message());
            return false;
        }
    ...
    public static function force_check() {
        delete_transient('ai_comment_moderator_github_release');
    ...
    public static function get_update_status() {
        $plugin_slug = 'ai-comment-moderator/ai-comment-moderator.php';
        $update_plugins = get_site_transient('update_plugins');
        
        if (isset($update_plugins->response[$plugin_slug])) {
            return array(
                'update_available' => true,
                'current_version' => AI_COMMENT_MODERATOR_VERSION,
...
// Initialize updater
function ai_comment_moderator_init_updater() {
    $plugin_file = AI_COMMENT_MODERATOR_PLUGIN_DIR . 'ai-comment-moderator.php';
    $github_repo = 'DroppedLink/ai-comment-moderator'; // username/repository
    
    new AI_Comment_Moderator_GitHub_Updater($plugin_file, $github_repo);
}
add_action('admin_init', 'ai_comment_moderator_init_updater');

// Handle AJAX check for updates
add_action('wp_ajax_ai_moderator_check_updates', 'ai_comment_moderator_ajax_check_updates');
function ai_comment_moderator_ajax_check_updates() {
    ...
    AI_Comment_Moderator_GitHub_Updater::force_check();
    $status = AI_Comment_Moderator_GitHub_Updater::get_update_status();
