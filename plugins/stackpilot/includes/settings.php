...
function sp_settings_page() {
	...
	$edit_env = $edit_key ? sp_get_env($edit_key) : null;

	if (isset($_POST['sp_save_env'])) {
		check_admin_referer('sp_settings_nonce');
		$env = sp_sanitize_env($_POST);
		sp_replace_env($env);
		echo '<div class="notice notice-success"><p>Environment saved.</p></div>';
	}

	if (isset($_POST['sp_delete_env']) && !empty($_POST['env_key'])) {
		check_admin_referer('sp_settings_nonce');
		sp_delete_env(sanitize_text_field($_POST['env_key']));
	...
	<div class="wrap">
		...
		<div class="notice notice-info is-dismissible">
			...
			<p><strong>Find your Endpoint</strong></p>
			<ul>
				<li>Enter your API URL and token below, then click "Load Endpoints" to list available environments.</li>
				<li>Alternatively, in the Portainer UI the URL often contains <code>endpointId=NUMBER</code>.</li>
			</ul>
		</div>
		...
		<form method="post">
			...
			<table class="form-table" role="presentation">
				<tr>
					<th scope="row"><label for="sp_env_name">Name</label></th>
					<td><input name="name" id="sp_env_name" type="text" class="regular-text" value="<?php echo esc_attr($edit_env['name'] ?? ''); ?>" required></td>
				</tr>
				<tr>
				...
				<tr>
					...
					<td>
						<select name="id" id="sp_env_endpoint_id" class="regular-text" required>
						...
						<button type="button" class="button" id="sp-fetch-endpoints">Load Endpoints</button>
				...
				<tr>
					...
					<td><label><input id="sp_env_tls_verify" name="tls_verify" type="checkbox" value="1" <?php echo !empty($edit_env['tls_verify']) ? 'checked' : ''; ?>> Enable SSL verification</label></td>
				</tr>
			</table>
			<p class="submit">
				<input type="submit" name="sp_save_env" class="button button-primary" value="Save Environment">
			</p>
		</form>
				...
				<tr>
					...
					<td>
						<form method="post" style="display:inline">
							...
							<input type="submit" name="sp_delete_env" class="button button-small" value="Delete" onclick="return confirm('Delete this environment?');">
						</form>
						<a class="button button-small" href="<?php echo esc_url(admin_url('admin.php?page=stackpilot-settings&edit=' . urlencode($env['key'] ?? ''))); ?>">Edit</a>
						<button class="button button-small sp-test-env" data-envkey="<?php echo esc_attr($env['key'] ?? ''); ?>">Test</button>
					</td>
				</tr>
		...
		<form method="post">
			...
			<table class="form-table" role="presentation">
				<tr>
					<th scope="row"><label for="sp_cache_ttl">Cache TTL (seconds)</label></th>
					<td><input name="sp_cache_ttl" id="sp_cache_ttl" type="number" class="small-text" min="0" value="<?php echo esc_attr((string) get_option('sp_cache_ttl', get_option('pv_cache_ttl', 30))); ?>"> <span class="description">0 to disable caching.</span></td>
				</tr>
				<tr>
	...
	(function($){
		...
		$(document).on('click', '#sp-fetch-endpoints', function(){
			var apiUrl = $('#sp_env_api_url').val();
			var apiToken = $('#sp_env_api_token').val();
			var tlsVerify = $('#sp_env_tls_verify').is(':checked');
			...
			$.post(ajaxurl, {
				action: 'sp_fetch_endpoints',
				nonce: '<?php echo esc_js(wp_create_nonce('sp_nonce')); ?>',
				api_url: apiUrl,
				api_token: apiToken,
				...
				if (r.data && r.data.length > 0) {
					r.data.forEach(function(endpoint){
						...
						if (endpoint.id == currentVal) { option.prop('selected', true); }
